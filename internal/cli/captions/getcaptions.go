...
var getCaptionsCmdMeta = []flagutil.FlagMeta{
	{FlagName: "media-id", Shorthand: "m", FieldPath: "MediaID", Kind: flagutil.FlagKindString, Optional: true, Description: "Find captions for a particular media by providing the media hashed ID"},
	...
	{FlagName: "sort-by", FieldPath: "SortBy", Kind: flagutil.FlagKindEnum, Optional: true, HasDefault: true, DefaultStr: "id", EnumValues: []string{"id", "created"}, Description: "Ordering. When using cursor pagination (see cursor param),\nonly `id` is supported.\n (options: id, created)"},
...
// initGetCaptionsCmd initializes the get-captions command.
func initGetCaptionsCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		Use:     "get",
		Short:   "List Captions",
		Long:    "Lists captions belonging to the account. This endpoint can also narrow down results\nto those belonging to a specific media.\n\n## Requires api token with one of the following permissions\n```\nRead all folder and media data\n```",
		Example: "  wistia captions get",
		RunE:    runGetCaptionsCmd,
	}
	flagutil.RegisterFlags(cmd, getCaptionsCmdMeta)
	if err := flagutil.ValidateMeta[operations.GetCaptionsRequest](getCaptionsCmdMeta); err != nil {
		return fmt.Errorf("invalid metadata for get-captions: %w", err)
	}
	parent.AddCommand(cmd)
	return nil
}

// runGetCaptionsCmd executes the get-captions command.
func runGetCaptionsCmd(cmd *cobra.Command, args []string) error {
	if usage.UsageRequested(cmd) {
		return usage.EmitSchema(cmd, cmd.OutOrStdout())
	}
	if interactive.ShouldPrompt(cmd, getCaptionsCmdMeta) {
		if err := interactive.PromptAndSetFlags(cmd, getCaptionsCmdMeta); err != nil {
			return err
		}
	}
	req, err := flagutil.BuildRequest[operations.GetCaptionsRequest](cmd, getCaptionsCmdMeta, "", "")
	if err != nil {
		return err
	}
	...
	res, err := s.Captions.GetCaptions(cmd.Context(), req, sdkOpts...)
	if err != nil {
