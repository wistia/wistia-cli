...
func initDeleteCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Long:    "Deletes the share link for a media, revoking the URL. A new share link\ncan be created later with a fresh URL by calling the show or update\nendpoint again.\n\n## Requires api token with one of the following permissions\n```\nRead, update & delete anything\n```",
		Example: "  wistia share-links delete --media-id <id>",
		RunE:    runDeleteCmd,
	}
	flagutil.RegisterFlags(cmd, deleteCmdMeta)
	if err := flagutil.ValidateMeta[operations.DeleteMediasMediaIDShareLinkRequest](deleteCmdMeta); err != nil {
...
func runDeleteCmd(cmd *cobra.Command, args []string) error {
	...
	req, err := flagutil.BuildRequest[operations.DeleteMediasMediaIDShareLinkRequest](cmd, deleteCmdMeta, "", "")
	if err != nil {
		return err
	}
	...
	res, err := s.ShareLinks.Delete(cmd.Context(), *req, sdkOpts...)
	if err != nil {
