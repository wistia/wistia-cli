...
var uploadFormCmdMeta = []flagutil.FlagMeta{
	...
	{FlagName: "url", Shorthand: "u", FieldPath: "URL", Kind: flagutil.FlagKindString, Required: true, Description: "The web location of the media file to import. [required]"},
...
// initUploadFormCmd initializes the upload-form command.
func initUploadFormCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Example: "  wistia media upload-form --url http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4",
		RunE:    runUploadFormCmd,
		Aliases: []string{"uf"},
	}
	flagutil.RegisterFlags(cmd, uploadFormCmdMeta)
	if err := flagutil.ValidateMeta[operations.PostFormRequest](uploadFormCmdMeta); err != nil {
		return fmt.Errorf("invalid metadata for upload-form: %w", err)
	}
...
// runUploadFormCmd executes the upload-form command.
func runUploadFormCmd(cmd *cobra.Command, args []string) error {
	if usage.UsageRequested(cmd) {
		return usage.EmitSchema(cmd, cmd.OutOrStdout())
	}
	if interactive.ShouldPrompt(cmd, uploadFormCmdMeta) {
		if err := interactive.PromptAndSetFlags(cmd, uploadFormCmdMeta); err != nil {
			return err
		}
	}
	request, err := flagutil.BuildRequest[operations.PostFormRequest](cmd, uploadFormCmdMeta, "", "body")
	if err != nil {
		return err
	}
	...
	res, err := s.Media.UploadForm(cmd.Context(), request, sdkOpts...)
	if err != nil {
