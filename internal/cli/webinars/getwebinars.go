...
var getWebinarsCmdMeta = []flagutil.FlagMeta{
...
// initGetWebinarsCmd initializes the get-webinars command.
func initGetWebinarsCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Example: "  wistia webinars get",
		RunE:    runGetWebinarsCmd,
	}
	flagutil.RegisterFlags(cmd, getWebinarsCmdMeta)
	if err := flagutil.ValidateMeta[operations.GetWebinarsRequest](getWebinarsCmdMeta); err != nil {
		return fmt.Errorf("invalid metadata for get-webinars: %w", err)
	}
	parent.AddCommand(cmd)
	return nil
}

// runGetWebinarsCmd executes the get-webinars command.
func runGetWebinarsCmd(cmd *cobra.Command, args []string) error {
	if usage.UsageRequested(cmd) {
		return usage.EmitSchema(cmd, cmd.OutOrStdout())
	}
	if interactive.ShouldPrompt(cmd, getWebinarsCmdMeta) {
		if err := interactive.PromptAndSetFlags(cmd, getWebinarsCmdMeta); err != nil {
			return err
		}
	}
	req, err := flagutil.BuildRequest[operations.GetWebinarsRequest](cmd, getWebinarsCmdMeta, "", "")
	if err != nil {
		return err
	}
	...
	res, err := s.Webinars.GetWebinars(cmd.Context(), req, sdkOpts...)
	if err != nil {
