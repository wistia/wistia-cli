...
func (s *AnalyticsMedia) GetEmbedLocationsTimeseries(ctx context.Context, request operations.GetAnalyticsMediasMediaIDEmbedLocationsTimeseriesRequest, opts ...operations.Option) (*operations.GetAnalyticsMediasMediaIDEmbedLocationsTimeseriesResponse, error) {
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/analytics/medias/{mediaId}/embed_locations_timeseries", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	hookCtx := hooks.HookContext{
		...
		BaseURL:          baseURL,
		Context:          ctx,
		OperationID:      "get_/analytics/medias/{mediaId}/embed_locations_timeseries",
		OAuth2Scopes:     nil,
		SecuritySource:   s.sdkConfiguration.Security,
	}
	...
	res := &operations.GetAnalyticsMediasMediaIDEmbedLocationsTimeseriesResponse{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			if o.SkipDeserialization == nil || !*o.SkipDeserialization {
				...
				var out []operations.GetAnalyticsMediasMediaIDEmbedLocationsTimeseriesResponseBody
				if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
					return nil, err
				}
