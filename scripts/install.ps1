#
# wistia CLI Installation Script for Windows
# This script downloads and installs the latest version of the wistia CLI
#
# Usage:
#   iwr -useb https://raw.githubusercontent.com/wistia/wistia-cli/main/scripts/install.ps1 | iex
#   or
#   Invoke-WebRequest -Uri https://raw.githubusercontent.com/wistia/wistia-cli/main/scripts/install.ps1 -UseBasicParsing | Invoke-Expression
#
# Options:
#   $env:WISTIA_CLI_INSTALL_DIR - Installation directory (default: $env:LOCALAPPDATA\Programs\wistia)
#   $env:WISTIA_CLI_VERSION     - Specific version to install (default: latest)
#
...
# Configuration
$Repo = "wistia/wistia-cli"
$BinaryName = "wistia.exe"
$DefaultInstallDir = Join-Path $env:LOCALAPPDATA "Programs\wistia"
$InstallDir = if ($env:WISTIA_CLI_INSTALL_DIR) { $env:WISTIA_CLI_INSTALL_DIR } else { $DefaultInstallDir }
$Version = if ($env:WISTIA_CLI_VERSION) { $env:WISTIA_CLI_VERSION } else { "latest" }
...
function Install-CLI {
    Write-ColorOutput "Installing wistia CLI..." -Color Green
    ...
    $archiveName = "wistia_Windows_$arch.zip"
    $downloadUrl = "https://github.com/$Repo/releases/download/$Version/$archiveName"

    Write-ColorOutput "Downloading from: $downloadUrl" -Color Cyan

    # Create temporary directory
    $tempDir = Join-Path $env:TEMP "wistia-install-$(New-Guid)"
    ...
    try {
        ...
        Write-ColorOutput "wistia $Version has been installed to $binaryPath" -Color Green
        ...
        Write-ColorOutput "Installation successful! Run 'wistia --help' to get started." -Color Green
